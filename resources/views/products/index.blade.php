    ...
    <div class="row">
        ...
        <div class="col-lg-3 mb-4">
                ...
                <div class="card-body p-0">
                    <div class="list-group list-group-flush">
                        <a href="{{ route('products.index') }}" 
                           class="list-group-item list-group-item-action {{ !request('category') ? 'active' : '' }}" aria-label="View all categories">
                            <i class="fas fa-th-large me-2"></i>All Categories
                        </a>
                        @foreach($categories as $category)
                            <a href="{{ route('products.index', ['category' => $category->slug]) }}" 
                               class="list-group-item list-group-item-action {{ request('category') == $category->slug ? 'active' : '' }}" aria-label="View products in {{ $category->name }} category">
            ...
            <div class="card mt-4 sidebar-card theme-card" role="region" aria-label="Price filter">
                ...
                <div class="card-body">
                    <form method="GET" action="{{ route('products.index') }}" aria-label="Price filter form">
                        @if(request('category'))
        ...
        <div class="col-lg-9">
            ...
            <div class="card mb-4">
                <div class="card-body">
                    <form method="GET" action="{{ route('products.index') }}" class="row g-3" aria-label="Search and sort form">
                        <div class="col-md-8">
                ...
                @forelse($products as $product)
                    <div class="product-card">
                        ...
                        <div class="product-info">
                            <div class="product-category category-glow">{{ $product->category->name }}</div>
                            ...
                            <div class="d-grid gap-2">
                                ...
                                <button type="button" class="btn btn-outline-danger btn-sm w-100 wishlist-btn" data-product-id="{{ $product->id }}" aria-label="{{ in_array($product->id, $userWishlists) ? 'Remove' : 'Add' }} {{ $product->name }} to wishlist">
                                    <i class="fa{{ in_array($product->id, $userWishlists) ? 's' : 'r' }} fa-heart me-1"></i>
                                    <span class="wishlist-btn-text">{{ in_array($product->id, $userWishlists) ? 'Remove from Wishlist' : 'Add to Wishlist' }}</span>
                                </button>
                            </div>
                        </div>
                    </div>
                @empty
                    <div class="col-12">
                        <div class="text-center py-5">
                            ...
                            <a href="{{ route('products.index') }}" class="btn btn-accent" aria-label="Clear all filters">Clear Filters</a>
                        </div>
                    </div>
                @endforelse
            </div>
