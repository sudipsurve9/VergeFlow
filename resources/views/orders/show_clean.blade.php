        ...
        <div class="col-12">
            <div class="order-header-card">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('orders.index') }}" class="btn-back-modern">
        ...
        <div class="col-lg-8">
                    ...
                    <div class="info-grid-modern">
                        <div class="info-item">
                            ...
                            <div class="info-content">
                                ...
                                <div class="info-value">{{ $order->order_number ?? 'ORD-' . str_pad($order->id, 6, '0', STR_PAD_LEFT) }}</div>
                            </div>
                        </div>
                        ...
                        <div class="info-item">
                            ...
                            <div class="info-content">
                                ...
                                <div class="info-value">{{ $order->payment_method ? ucfirst(str_replace(['_', 'cod'], [' ', 'Cash on Delivery'], strtolower($order->payment_method))) : 'Not specified' }}</div>
                            </div>
                        </div>
                        ...
                        <div class="info-item">
                            ...
                            <div class="info-content">
                                ...
                                <div class="info-value">{{ $order->created_at->format('F d, Y \a\t g:i A') }}</div>
                            </div>
                        </div>
                        ...
                        <div class="info-item">
                            ...
                            <div class="info-content">
                                ...
                                <div class="info-value">
                                    <span class="status-badge payment-{{ $order->payment_status ?? 'pending' }}">{{ $order->payment_status ? ucfirst($order->payment_status) : 'Pending' }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="info-item">
                            ...
                            <div class="info-content">
                                ...
                                <div class="info-value">
                                    <span class="status-badge order-{{ $order->status ?? 'pending' }}">{{ $order->status ? ucfirst($order->status) : 'Pending' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="modern-card order-items-card mb-4" role="region" aria-label="Order items">
                <div class="card-header-modern">
                    ...
                    <div class="items-count-badge">{{ $order->items->count() }} {{ $order->items->count() == 1 ? 'Item' : 'Items' }}</div>
                </div>
                <div class="card-body-modern">
                            ...
                            <div class="item-image-wrapper">
                                @if($item->product && $item->product->image)
                                    ...
                                    <div class="item-placeholder" style="display: none;">
                                        ...
                                        <small class="mt-1">{{ $item->product->name }}</small>
                                    </div>
                                @else
                                    <div class="item-placeholder">
                                        <i class="fas fa-cube"></i>
                                        <small class="mt-1">{{ $item->product ? $item->product->name : 'Product' }}</small>
                                    </div>
                                @endif
                            </div>
                            
                            <div class="item-details-modern">
                                <h4 class="item-name-modern">{{ $item->product ? $item->product->name : 'Product not found' }}</h4>
                                <div class="item-meta">
                            ...
                            <div class="item-pricing-modern">
                                ...
                                <div class="total-price">
                                    ...
                                    <span class="price-value-total">
                                        <i class="fas fa-rupee-sign"></i>{{ number_format($item->total ?? ($item->price * $item->quantity), 2) }}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="address-content">
                            ...
                            <div class="address-details">
                                @if($order->shippingAddress)
                                    <div class="address-line">{{ $order->shippingAddress->address_line_1 }}</div>
                                    @if($order->shippingAddress->address_line_2)
                                        <div class="address-line">{{ $order->shippingAddress->address_line_2 }}</div>
                                    @endif
                                    <div class="address-line">{{ $order->shippingAddress->city }}, {{ $order->shippingAddress->state }} {{ $order->shippingAddress->postal_code }}</div>
                                    <div class="address-line country">{{ $order->shippingAddress->country }}</div>
                                @else
                                    <div class="address-line no-address">{{ $order->shipping_address ?? 'No shipping address provided' }}</div>
                                @endif
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="timeline-item">
                            ...
                            <div class="timeline-content">
                                ...
                                <small class="text-muted">
                                    <i class="fa-solid fa-calendar me-1"></i>{{ $history->created_at->format('M d, Y \a\t g:i A') }}
                                </small>
                            </div>
                        </div>
                        @endforeach
            ...
            <div class="modern-card summary-card mb-4" role="region" aria-label="Order summary">
                ...
                <div class="card-body-modern">
                    <div class="summary-breakdown">
                        <div class="summary-line">
                            ...
                            <div class="summary-value">
                                <i class="fas fa-rupee-sign me-1"></i>{{ number_format($order->subtotal ?? $order->items->sum(function($item) { return $item->total ?? ($item->price * $item->quantity); }), 2) }}
                            </div>
                        </div>
                        ...
                        <div class="summary-line">
                            ...
                            <div class="summary-value">
                                <i class="fas fa-rupee-sign me-1"></i>{{ number_format($order->shipping_amount ?? 0, 2) }}
                            </div>
                        </div>
                        ...
                        <div class="summary-line">
                            ...
                            <div class="summary-value">
                                <i class="fas fa-rupee-sign me-1"></i>{{ number_format($order->tax_amount ?? 0, 2) }}
                            </div>
                        </div>
                        ...
                        <div class="summary-total-line">
                            ...
                            <div class="total-value">
                                <i class="fas fa-rupee-sign me-1"></i>{{ number_format($order->total_amount ?? ($order->subtotal ?? $order->items->sum(function($item) { return $item->total ?? ($item->price * $item->quantity); })) + ($order->shipping_amount ?? 0) + ($order->tax_amount ?? 0), 2) }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="status-indicator-wrapper mt-4">
                        <div class="status-indicator status-{{ $order->status ?? 'pending' }}">
                            ...
                            <div class="status-text">
                                <div class="status-title">{{ ucfirst($order->status ?? 'pending') }}</div>
